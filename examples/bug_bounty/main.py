...
cache_manager = CacheManager(backend=CacheManager.CacheBackend.MEMORY, max_size=2000)
db_manager = DatabaseManager()
kali_optimizer = KaliOptimizer(subprocess_handler)
workflow_engine = WorkflowEngine(cache_manager, subprocess_handler)
ai_agent = AIAgent()
...
async def run_system_diagnostics():
    ...
    # Check Kali tools
    system_status['kali_tools'] = await kali_optimizer.check_all_tools()
    
    # Get system resources
    system_status['system_resources'] = kali_optimizer.get_system_resources()
...
async def get_behind_scenes():
    ...
    try:
        with open("templates/behind_scenes.html", "r", encoding="utf-8") as f:
            return HTMLResponse(content=f.read())
    except FileNotFoundError:
...
async def start_hunt(request: TargetRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        workflow_id = f"hunt_{int(time.time())}"
        ...
        return {
            ...
            'message': 'Bug hunting workflow started successfully',
            'dashboard_url': f'/enhanced?workflow_id={workflow_id}'
        }
...
async def run_workflow(workflow_id: str, request: TargetRequest):
    ...
    try:
        ...
        tools_config = await kali_optimizer.optimize_for_target(request.target, ai_analysis)
        workflow['steps'].append({
        ...
        workflow['logs'].append({
            'timestamp': datetime.now().isoformat(),
            'level': 'info',
            'message': 'Step 3: Reconnaissance'
        })
        
        recon_results = await workflow_engine.run_reconnaissance(request.target, tools_config)
        workflow['steps'].append({
            'name': 'Reconnaissance',
            'status': 'completed',
        ...
        workflow['logs'].append({
            'timestamp': datetime.now().isoformat(),
            'level': 'info',
            'message': 'Step 4: Vulnerability Scanning'
        })
        
        vuln_results = await workflow_engine.run_vulnerability_scan(request.target, recon_results)
        workflow['steps'].append({
        ...
        report = await workflow_engine.generate_report(workflow_id, analysis_results)
        workflow['steps'].append({
...
async def get_performance_metrics():
    ...
    return {
        ...
        'system_resources': kali_optimizer.get_system_resources()
    }
